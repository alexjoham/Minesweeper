...
public:
    ...
    bool contains(int cx, int cy) const { return cx == x_ && cy >= y_ && cy < y_ + w_; }

    void setColor(std::string new_color) { color_ = std::move(new_color); }
    ...
    Button(int x, int y, int w, std::string color) : x_(x), y_(y), w_(w), color_(std::move(color)) { id = ++ID; }
...
public:
    LabelButton(int x, int y, int w, std::string label, std::string color)
        : Button(x, y, w, color), label_(std::move(label)) {}
...
public:
    UnicodeButton(int x, int y, int w, std::string code, std::string color)
        : Button(x, y, w, color), code_(std::move(code)) {}
    void draw() const override {
    ...
    void setCode(std::string new_code) { code_ = std::move(new_code); }
};
