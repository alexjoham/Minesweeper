...
public:
    Tui(int origin_row, int origin_col) noexcept : origin_row_(origin_row), origin_col_(origin_col) {}
    /**
    ...
    static std::string_view colour(const Playerield& c) {
        if (c.hidden)
            return c.flagged ? "\x1b[93m" : "\x1b[97m";
        switch (c.fieldType) {
            case FieldType::HIDDEN:
                return "\x1b[97m";
            case FieldType::MINE:
    ...
    std::optional<BoardCoord> boardCellAt(int screen_row, int screen_col) const {
        ...
        if (row >= Game::kFieldSize || column >= Game::kFieldSize)
            return std::nullopt;
