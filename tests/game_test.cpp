...
Game::Board getEmptyRevealedBoard() {
    Game::Board board;
    for (auto& row : board)
        std::fill(std::begin(row), std::end(row), Playerield{false, false, FieldType::NEUTRAL});
    return board;
}
...
TEST(GameTest, MakeMoveDoesNotRevealDuplicateCells) {
    ...
    for (const RevealedCell& cell : revealed_cells) {
        ...
        EXPECT_TRUE(revealed_set.emplace(coord_pair).second)
            << "duplicate cell (" << coord_pair.first << ", " << coord_pair.second << ")";
    }
}
...
TEST(GameTest, RevealAllOnlyRevealsStillHiddenCells) {
    ...
    bool contains_already_revealed_cell =
        std::any_of(revealed_by_reveal_all.begin(), revealed_by_reveal_all.end(),
